...
public class PlayerDamage : MonoBehaviour
{
    ...
    public void TakeDamage(int AttackerId, string a_AttTeam = "blue")
    {
        ...
        {
            //자신을 파괴시킨 적 탱크의 스코어를 증가시키는 함수를 호출
            // SaveKillCount(AttackerId);   //<-- 지금 죽은 애도, 쏜 애도 내가(IsMine) 될 수는 없다.
            //지금 죽는 IsMine 탱크 입장에서는 
            //아바타들 중에서 AttackerId 를 찾아서 KillCount를 증가 시켜 주어야 한다.
    ...
    void SetTankVisible(bool isVisible)
    {
        ...
        {
            if (isVisible == false)
                _BoxColl.gameObject.layer = LayerMask.NameToLayer("DieTank");
            else
                _BoxColl.gameObject.layer = LayerMask.NameToLayer("Default");
        }
